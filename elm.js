...
var $author$project$Text$cipherChallangeText = 'BT JPX RMLX PCUV AMLX ICVJP IBTWXVR CI M LMT’R PMTN, MTN YVCJX CDXV MWMBTRJ JPX AMTNGXRJBAH UQCT JPX QGMRJXV CI JPX YMGG CI JPX HBTW’R QMGMAX; MTN JPX HBTW RMY JPX QMVJ CI JPX PMTN JPMJ YVCJX. JPXT JPX HBTW’R ACUTJXTMTAX YMR APMTWXN, MTN PBR JPCUWPJR JVCUFGXN PBL, RC JPMJ JPX SCBTJR CI PBR GCBTR YXVX GCCRXN, MTN PBR HTXXR RLCJX CTX MWMBTRJ MTCJPXV. JPX HBTW AVBXN MGCUN JC FVBTW BT JPX MRJVCGCWXVR, JPX APMGNXMTR, MTN JPX RCCJPRMEXVR. MTN JPX HBTW RQMHX, MTN RMBN JC JPX YBRX LXT CI FMFEGCT, YPCRCXDXV RPMGG VXMN JPBR YVBJBTW, MTN RPCY LX JPX BTJXVQVXJMJBCT JPXVXCI, RPMGG FX AGCJPXN YBJP RAMVGXJ, MTN PMDX M APMBT CI WCGN MFCUJ PBR TXAH, MTN RPMGG FX JPX JPBVN VUGXV BT JPX HBTWNCL.';
var $author$project$Main$initialModel = {cipher: '', pairs: _List_Nil, plain: '', text: $author$project$Text$cipherChallangeText};
...
var $author$project$Main$update = F2(
	function (msg, model) {
		switch (msg.$) {
			case 'SelectCipherLetter':
				var cipherLetter = msg.a;
				return _Utils_update(
					model,
					{cipher: cipherLetter});
			case 'SelectPlainLetter':
				var plainLetter = msg.a;
				return _Utils_update(
					model,
					{plain: plainLetter});
			case 'GenerateCipherLetterPair':
				return _Utils_update(
					model,
					{
						cipher: '',
						pairs: A2(
							$elm$core$List$cons,
							_Utils_Tuple2(model.cipher, model.plain),
							model.pairs),
						plain: ''
					});
			case 'RemoveCipherLetterPair':
				...
				var cipher = _v1.a;
				var plain = _v1.b;
				var shouldBeKept = function (_v2) {
					var targetCipher = _v2.a;
					var targetPlain = _v2.b;
					return (!_Utils_eq(cipher, targetCipher)) && (!_Utils_eq(plain, targetPlain));
				};
				return _Utils_update(
					model,
					{
...
var $author$project$Main$generateChars = function (unicodes) {
	return A2($elm$core$List$map, $elm$core$Char$fromCode, unicodes);
};
var $author$project$Main$genAllUpperCaseStr = A2(
	$elm$core$List$map,
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('btn btn-sm-red'),
						$elm$html$Html$Events$onClick(
						$author$project$Main$RemoveCipherLetterPair(
							_Utils_Tuple2(cipher, plain)))
					]),
				_List_fromArray(
					[
...
var $author$project$Main$cipherToPlainView = function (model) {
...
var $author$project$Main$genAllLowerCaseStr = A2(
	$elm$core$List$map,
	$elm$core$String$fromChar,
	$author$project$Main$generateChars(
...
var $author$project$Main$textView = function (model) {
	var textArr = A2($elm$core$String$split, '', model.text);
	var pairs = $elm$core$Dict$fromList(model.pairs);
	var letterView = function (letter) {
		var transformedLetter = A2(
			$elm$core$Maybe$withDefault,
			letter,
			A2($elm$core$Dict$get, letter, pairs));
		var shouldHighlight = _Utils_eq(transformedLetter, model.cipher);
		return A2(
			$elm$html$Html$span,
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							_Utils_Tuple2('highlight', shouldHighlight)
						]))
				]),
			_List_fromArray(
				[
					$elm$html$Html$text(transformedLetter)
				]));
	};
	return A2(
		$elm$html$Html$p,
		_List_fromArray(
			[
				$elm$html$Html$Attributes$class('text letter-spacing-l')
			]),
		A2($elm$core$List$map, letterView, textArr));
};
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$author$project$Main$cipherToPlainView(model),
						...
						_List_fromArray(
							[
								$author$project$Main$textView(model)
							]))
					]))
			]));
};
